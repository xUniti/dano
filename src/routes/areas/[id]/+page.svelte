...
<PageHeader title={area?.name ?? "Area"} subtitle="Life area">
  <a href="/areas" class="rounded-lg border border-fg/10 px-3 py-1.5 text-xs text-fg/70 hover:bg-fg/5 hover:text-fg">← Areas</a>
</PageHeader>
...
{:else if notFound || !area}
  <div class="p-6 text-sm text-fg/40">Area not found. <a class="text-accent" href="/areas">Back to areas</a></div>
{:else}
  <div class="p-6">
    <!-- header row -->
    <div class="mb-6 flex items-center gap-3">
      ...
      <div class="flex items-center gap-1">
        {#each palette as c (c)}
          <button type="button" onclick={() => setColor(c)} aria-label="color" class="h-4 w-4 rounded-full {area.color === c ? 'ring-2 ring-fg/70' : ''}" style="background:{c}"></button>
        {/each}
      </div>
      <button type="button" onclick={archive} class="rounded-md px-2 py-1 text-xs text-fg/40 hover:bg-amber-500/15 hover:text-amber-300">Archive</button>
    </div>
    ...
    <div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
      {#each [["Projects", stats.projects], ["Tasks", stats.total], ["Done", stats.done], ["Avg progress", stats.avg + "%"]] as [label, value] (label)}
        <div class="rounded-xl border border-fg/10 bg-fg/[0.02] p-3">
          <div class="text-lg font-semibold text-fg/90">{value}</div>
    ...
    <div class="mb-3 flex items-center gap-2">
      <h2 class="text-sm font-semibold tracking-tight text-fg/85">Projects</h2>
    </div>
    <div class="mb-3 flex gap-2">
      ...
      <button type="button" onclick={addProject} class="rounded-lg bg-accent/80 px-4 py-2 text-sm font-medium text-fg hover:bg-accent">Add</button>
    </div>
    {#if projects.length === 0}
      <p class="text-sm text-fg/35">No projects in this area yet.</p>
    {:else}
