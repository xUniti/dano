...
<div
  class="group flex items-center gap-3 rounded-lg border border-white/5 bg-white/[0.02] px-3 py-2 hover:border-white/10"
>
  <input
    type="checkbox"
  ...
  <div class="min-w-0 flex-1">
    {#if editing}
      ...
      <input
        ...
        autofocus
        class="w-full rounded bg-white/10 px-1 py-0.5 text-sm outline-none"
      />
    {:else}
      <button
        type="button"
  ...
  {#if projectName}
    <span class="shrink-0 rounded bg-white/5 px-1.5 py-0.5 text-[10px] text-white/40">{projectName}</span>
  {/if}
  ...
  <div class="flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100 focus-within:opacity-100">
    <input
      ...
      onchange={setDue}
      class="rounded bg-white/5 px-1 py-0.5 text-[11px] text-white/60 [color-scheme:dark]"
    />
    <select
      value={task.status}
      onchange={setStatus}
      class="rounded bg-white/5 px-1 py-0.5 text-[11px] text-white/60"
    >
    ...
    <button
      type="button"
      onclick={remove}
      class="rounded px-1.5 py-0.5 text-[11px] text-white/40 hover:bg-red-500/20 hover:text-red-300"
